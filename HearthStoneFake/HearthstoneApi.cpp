...
CardSpec HearthstoneApi::ParseToCard(std::string_view JsonContent)
{
	auto Converted = json::parse(JsonContent).as_object();

	CardSpec Result{};

	if (auto Value = Converted[CardSpec::KEY_CARD_ID].if_int64())
		Result.CardId = static_cast<int>(*Value);
	if (auto Value = Converted[CardSpec::KEY_COLLECTIBLE].if_int64())
		Result.Collectible = static_cast<bool>(*Value);
	if (auto Value = Converted[CardSpec::KEY_SLUG].if_string())
		Result.Slug = *Value;
	if (auto Value = Converted[CardSpec::KEY_CLASS_ID].if_int64())
		Result.ClassId = static_cast<int>(*Value);
	if (auto Value = Converted[CardSpec::KEY_MULTI_CLASS_IDS].if_array())
		Result.MultiClassIds = JsonArrayToVector(*Value);
	if (auto Value = Converted[CardSpec::KEY_SPELL_SCHOOL_ID].if_int64())
		Result.SpellSchoolId = static_cast<int>(*Value);
	if (auto Value = Converted[CardSpec::KEY_CARD_TYPE_ID].if_int64())
		Result.CardTypeId = static_cast<int>(*Value);
	if (auto Value = Converted[CardSpec::KEY_CARD_SET_ID].if_int64())
		Result.CardSetId = static_cast<int>(*Value);
	if (auto Value = Converted[CardSpec::KEY_RARITY_ID].if_int64())
		Result.RarityId = static_cast<int>(*Value);
	if (auto Value = Converted[CardSpec::KEY_MANA_COST].if_int64())
		Result.ManaCost = static_cast<int>(*Value);
	if (auto Value = Converted[CardSpec::KEY_NAME].if_string())
		Result.Name = *Value;
	if (auto Value = Converted[CardSpec::KEY_TEXT].if_string())
		Result.Text = *Value;
	if (auto Value = Converted[CardSpec::KEY_PARENT_ID].if_int64())
		Result.ParentId = static_cast<int>(*Value);
	if (auto Value = Converted[CardSpec::KEY_CHILD_IDS].if_array())
		Result.ChildIds = JsonArrayToVector(*Value);
	if (auto Value = Converted[CardSpec::KEY_KEYWORD_IDS].if_array())
